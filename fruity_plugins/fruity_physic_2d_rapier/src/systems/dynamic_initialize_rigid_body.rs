...
pub fn dynamic_initialize_rigid_body(
    rapier_2d_service: Ref<Rapier2dService>,
    query: Query<(
        With<Transform2d>,
        WithMut<DynamicRigidBody>,
        WithExtension<RectCollider, RapierRectCollider>,
    )>,
) -> StartupDisposeSystemCallback {
    let handle = query.on_created(
        move |(transform, mut dynamic_rigid_body, (_, rapier_collider))| {
            let translation = transform.transform.translation();
            let rotation = transform.transform.rotation();

            let rigid_body = RigidBodyBuilder::dynamic()
                .gravity_scale(dynamic_rigid_body.gravity_scale)
                .can_sleep(dynamic_rigid_body.can_sleep)
                .ccd_enabled(dynamic_rigid_body.ccd_enabled)
            ...
            dynamic_rigid_body.handle = Some(rigid_body_handle.into_raw_parts());
