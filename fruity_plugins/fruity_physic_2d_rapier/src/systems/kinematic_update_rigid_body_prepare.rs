...
pub fn kinematic_update_rigid_body_prepare(
    rapier_2d_service: Ref<Rapier2dService>,
    query: Query<(With<Transform2d>, With<KinematicRigidBody>)>,
) {
    query.for_each(move |(transform, kinematic_rigid_body)| {
        if let Some(rigid_body_handle) = kinematic_rigid_body.handle {
                    ...
                    ))
            {
                ...
                rapier_rigid_body
                    .set_next_kinematic_translation(vector![translation.x, translation.y]);
                rapier_rigid_body.set_next_kinematic_rotation(rotation);
            }
        }
    })
}
